...
# This model attempts to concatenate the category info everywhere
class TransformerModel_with_Category_edited(nn.Module):
    ...
    def forward(self, src: Tensor, category: Tensor, src_mask: Tensor = None) -> Tensor:
        ...
        output = self.decoder(torch.cat((output, cat.expand(output.size()[0], output.size()[1], 200)), dim=2))
        return output
...
class TransformerEncoderLayer_edited(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model*2, nhead, dropout=dropout, batch_first=batch_first,
                                            **factory_kwargs)
        ...
        self.norm1 = nn.LayerNorm(d_model*2, eps=layer_norm_eps, **factory_kwargs)
        self.norm2 = nn.LayerNorm(d_model, eps=layer_norm_eps, **factory_kwargs)
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)

        self.mid_linear = nn.Linear(d_model*2, d_model)
    ...
    def forward(
        ...
        why_not_sparsity_fast_path = 'nope'
        if not src.dim() == 3:
            why_not_sparsity_fast_path = f"input not batched; expected src.dim() of 3 but got {src.dim()}"
        elif self.training:
        ...
        x = src
        x = torch.cat((x, cat.expand(src.size()[0], src.size()[1], 200)), dim=2)
        if self.norm_first:
